...
public class ArrayMap implements Map {
	...
	public Object put(Object key, Object value) {
		...
		// 모든 entry를 돌면서 i번째 entry가 key와 같다면 value+1 을 넣어준다.
		for (int i = 0; i < this.size; i++) {
			if (this.entryArray[i].getKey().equals(key)) {
				Integer oldValue = (Integer) this.entryArray[i].getValue();
				this.entryArray[i].setValue(oldValue + 1);
				return oldValue;
			}
		}
